...
class CustomPipelineOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_value_provider_argument(
            ...
            default = '{}-analytics-data-lake/Firebase/Notification_Created/Notification_Created_testing.json'.format(env))
        parser.add_value_provider_argument(
            '--output',
            type=str,
...
def streaming_pipeline(project, region="us-central1"):
    ...
    subscription = "projects/{}/subscriptions/Notification_Created".format(project)
    bucket = "gs://{}-analytics-data-lake/Firebase/Notification_Created/".format(env)
    ...
    class WriteToGCS(beam.DoFn):
        def __init__(self):
            self.outdir = "gs://{}-analytics-data-lake/Firebase/Notification_Created/".format(env)
        ...
        def process(self, element):
            ...
            file_prefix = "Notification_Created_Output_" + timestampStr + '.json' 
            writer = FileSystems.create(self.outdir + file_prefix, 'text/plain')
            writer.write(element.encode())
            writer.close()

    subscription = "projects/{}/subscriptions/Notification_Created".format(project)
    topic = "projects/{}/topics/notification-created".format(project)
    bucket = "gs://{}-analytics-data-lake/Firebase/Notification_Created/".format(env)
