...
def streaming_pipeline(project, region="us-central1"):
    ...
    subscription = "projects/{}/subscriptions/Content_Published".format(project)
    bucket = "gs://{}-analytics-data-lake/Firebase/Content_Published/".format(env)
    ...
    class WriteToGCS(beam.DoFn):
        def __init__(self):
            self.outdir = "gs://{}-analytics-data-lake/Firebase/Content_Published/".format(env)
        ...
        def process(self, element):
            ...
            file_prefix = "Content_Published_Output_" + timestampStr + '.json' 
            writer = FileSystems.create(self.outdir + file_prefix, 'text/plain')
            writer.write(element.encode())
            writer.close()

    subscription = "projects/{}/subscriptions/Content_Published".format(project)
    topic = "projects/{}/topics/content-published".format(project)
    bucket = "gs://{}-analytics-data-lake/Firebase/Content_Published/".format(env)
